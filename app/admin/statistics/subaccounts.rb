ActiveAdmin.register Click, as: 'SubAccount' do
  ...
  index do
    ...
    column :decline
    column :money_wait
    column :money_approve
    column :money_decline
  end
    ...
    def scoped_collection
      Click.left_joins(:conversions)
          .select(
                  ...
                  'sum(case when conversions.status = 1 then conversions.ext_payout::int else 0 end) money_wait',
                  'sum(case when conversions.status = 2 then conversions.ext_payout::int else 0 end) money_approve',
                  'sum(case when conversions.status = 3 then conversions.ext_payout::int else 0 end) money_decline'
          )
    end
  end
end
